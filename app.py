...
FESTIVAL_DATES = {
    'venerdi': date(2025, 7, 12),
    'sabato': date(2025, 7, 13),
    'domenica': date(2025, 7, 14)
}
MAX_DAILY_CAPACITY = 200
GENRES = ["Rock", "Pop", "Jazz", "Electronic", "Folk", "Hip-Hop", "Classical", "Blues"]

app = Flask(__name__)
app.config["SECRET_KEY"] = "MadnessFestival2025SecretKey"
...
TICKET_PRICES = {'daily': 50.0, '2days': 90.0, 'full': 130.0}
DATE_TO_DAY = {
    '2025-07-12': 'venerdi',    
    '2025-07-13': 'sabato',     
    '2025-07-14': 'domenica'    
...
def require_participant():
    """Verifica se l'utente corrente è un partecipante autorizzato"""
    if not current_user.is_authenticated or current_user.user_type != 'participant':
        flash("Solo i partecipanti possono accedere a questa sezione", "danger")
        return redirect(url_for("home"))
    return None

def require_organizer():
    """Verifica se l'utente corrente è un organizzatore autorizzato"""
    if not current_user.is_authenticated or current_user.user_type != 'organizer':
        flash("Solo gli organizzatori possono accedere a questa sezione", "danger")
        return redirect(url_for("home"))
    return None
...
@app.route("/")
def home():
    """Pagina principale con elenco performance filtrabili"""
    ...
    # Carica palchi solo se non si sta filtrando
...
def performance_detail(id):
    """Pagina dettaglio performance individuale"""
...
def register_post():
    """Elabora registrazione nuovo utente"""
...
def login():
    ...
    # Crea oggetto utente per Flask-Login
    user = User(
        id=user_db['id'],
        email=user_db['email'],
...
@login_required
def profile():
    """Dashboard utente - viste diverse per partecipanti vs organizzatori"""
    if current_user.user_type == 'participant':
        ...
        stats = tickets_dao.get_festival_stats()
...
@login_required
def buy_ticket():
    """Form acquisto biglietti con prezzi dinamici e disponibilità"""
    auth_check = require_participant()
    ...
    # Definisci opzioni biglietti con logica di business
    ticket_types = [
        {
            'id': 'daily',         
            'value': 'daily', 
...
@login_required
def buy_ticket_post():
    """Elabora acquisto biglietti con validazione"""
    auth_check = require_participant()
    if auth_check: return auth_check
    ...
    # Diverse regole di validazione per tipo di biglietto
    ...
    # Verifica disponibilità e regole di acquisto
    ...
    # Elabora giorni per salvataggio database
    if ticket_type == 'full':
    ...
    # Logica prezzi
    price = float(TICKET_PRICES[ticket_type])
...
@login_required
def new_performance():
    """Form creazione performance per organizzatori"""
...
def new_performance_post():
    ...
    # nessun conflitto di programmazione
    ...
    form_data['published'] = False  # Bozza per default
...
def edit_performance(id):
    """Modifica bozza performance - solo il proprietario può modificare performance non pubblicate"""
    auth_check = require_organizer()
    if auth_check: return auth_check
...
def edit_performance_post(id):
    """Aggiorna performance con validazione"""
    auth_check = require_organizer()
    if auth_check: return auth_check
    ...
    # Verifica conflitti escludendo la performance corrente
...
def delete_performance_route(id):
    """Elimina bozza performance - solo le performance non pubblicate possono essere eliminate"""
    auth_check = require_organizer()
    if auth_check: return auth_check
...
def load_user(user_id):
    ...
    if db_user:
        return User(
            ...
            profile_image=db_user['profile_image'] if 'profile_image' in db_user else None
        )
    return None
